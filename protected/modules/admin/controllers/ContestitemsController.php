...
class ContestitemsController extends Controller
{
    ...
    public function accessRules()
    {
        return array(
            array('allow',
                'actions'=>array('index', 'delete', 'admin', 'update', 'add', 'search', 'view', 'approve', 'decline'),
                'users'=>array('denis@ekimov.su'),
            ),
    ...
    public function actionApprove($id) {
        ...
        $contest_item = $this->loadModel($id);
        $contest_item->status = 1;

        if($contest_item->save())
            echo "success";
        else
            echo var_export($contest_item->getErrors(), true);
    }
    ...
    public function actionCreate()
    {
        ...
        if(isset($_POST['ContestItems']))
        {
            $model->attributes=$_POST['ContestItems'];
            if($model->save())
    ...
    {
        $dataProvider=new CActiveDataProvider('ContestItems');
    ...
    {
        $model=new ContestItems('search');
        $model->unsetAttributes();  // clear any default values
        if(isset($_GET['ContestItems']))
            $model->attributes=$_GET['ContestItems'];
    ...
    {
        $model=ContestItems::model()->findByPk($id);
        if($model===null)
